...
- **Projects are plain JSON** (`.flograph`) — diffable and small. Node output
  caches live in a side-car `<project>.flograph.cache/` folder, so reopening
  a project restores its results without a re-run.
