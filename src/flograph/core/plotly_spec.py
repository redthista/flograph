"""Every chart Plotly Express draws, as one declarative parameter set.

Two nodes plot with Plotly — **Show Plotly** and **Chart per Value
(Plotly)** — and both had grown their own small copy of "seven kinds, x, y
and a colour". This module is the single description of what a Plotly chart
can be, so the two nodes offer the same settings and can never drift apart:
a chart type added here appears on both.

Nothing here draws, imports plotly, or touches Qt. It answers three
questions and hands the answers to a node's `run()`:

* **What can be set?** `params()` returns the PARAMS list — one row per
  Plotly Express argument worth exposing, each carrying the `visible_when`
  that hides it unless the chosen chart actually takes it.
* **What does the user's setting mean to plotly?** `build()` turns the
  stored params into the keyword arguments for `px.<kind>()`, resolving
  blanks, sentinels and paired min/max boxes.
* **What is left over?** `layout_updates()` covers the handful of settings
  that are figure *layout*, not chart data — bar mode and its friends.

## The kind table is generated, not typed

`KIND_ARGS` lists, per chart kind, exactly which arguments that Plotly
Express function accepts. It was generated from `inspect.signature` and is
checked against the installed plotly by `tests/test_plotly_spec.py`, which
fails if plotly gains or drops an argument. Everything else is derived from
it: a param declares which px argument it feeds and `params()` works out
the chart kinds that argument belongs to. So the properties panel can never
offer a setting the chosen chart would reject, and nobody has to maintain a
second list by hand.

`build()` filters against the same table on the way out, which is the real
safety net: passing an unknown keyword to a px function raises, and a
saved flow whose kind was switched after the fact would otherwise fail
rather than simply ignore the settings that no longer apply. The names it
dropped come back with the kwargs so the node can log them.

## Maps are deliberately absent

The ten map functions (`scatter_map`, `choropleth`, `density_map`, the geo
and mapbox variants) need latitude/longitude or ISO country codes, and the
tile-backed ones need network access, so they fail in ways unlike every
other chart here — a blank map rather than an error. They belong to their
own node with its own guidance, not to a kind dropdown. `imshow` is out for
the same sort of reason: it takes an image or an array, not a table.

Also not exposed, on purpose: `custom_data` (it only feeds callbacks, and
there are none), the per-value `*_map`/`*_sequence` overrides other than
the colour ones (they need a value-to-style mapping widget that does not
exist), and the facet spacings (fiddly, and the defaults are computed from
the facet count).

## More options

There are seventy-odd settings and no chart wants more than a third of
them. Beyond the per-kind gating, each row is either basic or advanced;
the advanced ones appear only when **More options** is ticked. That toggle
is `cosmetic`, so revealing the rest of the panel does not re-run the
chart.
"""
...
#: Value skips its shared Y scale for these, and `build()` fills in `x`
...
#: rather than a palette name, so the name has to be looked up; the lookup
#: lives in `build()` because this module never imports plotly.
COLOR_SEQUENCES: tuple[str, ...] = (
...
UNIVERSAL_ARGS: frozenset[str] = frozenset({
    "title", "subtitle", "template", "labels", "width", "height",
})


def kinds_taking(*args: str) -> list[str]:
    ...
    """
    if any(a in UNIVERSAL_ARGS for a in args):
        return list(KINDS)
    return [k for k in KINDS if any(a in KIND_ARGS[k] for a in args)]


# Every row below is a ParamSpec dict plus, at most, these three private
# keys, which `params()` strips before handing the row to the node:
#
#   "arg"      the px argument this row feeds. Decides which kinds show it
#              (via kinds_taking) and how build() emits it.
...
_ROWS: list[dict[str, Any]] = [
    {"name": "kind", "type": "choice", "label": "Kind",
     "options": list(KINDS), "default": "line"},
    ...
    # kind build() leaves it to layout_updates(); layout.barnorm means the
    # same thing.
    {"name": "barnorm", "type": "choice", "label": "Bar totals",
     "options": ["none", "fraction", "percent"], "default": "none",
     "kinds": sorted(set(kinds_taking("barnorm")) | {"bar"},
                     key=KINDS.index),
     "advanced": True},
...
#: Rows that feed a px argument, keyed by param name — used by build().
...
def params(names: Optional[list[str]] = None) -> list[dict[str, Any]]:
    """The PARAMS list for a node that draws with Plotly Express.

    `names` limits and orders the result, for a node that wants only part
    of the set; the default is every row. Each row comes back with its
    `visible_when` filled in — the chart kinds that accept its argument,
    ANDed with the "More options" tick for an advanced row — and with this
    module's private keys removed, so the result is a plain PARAMS list a
    node script can hand straight to flograph.
    """
    rows = _ROWS if names is None else [r for r in _ROWS if r["name"] in names]
    if names is not None:
        rows = sorted(rows, key=lambda r: names.index(r["name"]))
    out = []
    for row in rows:
        ...
        if kinds is None and "arg" in row:
            kinds = kinds_taking(row["arg"])
        if kinds is not None and len(kinds) < len(KINDS):
...
def default_columns(table, values: dict[str, Any],
    ...
    module knows nothing about, such as Chart per Value's split.
    """
    spoken_for = set(exclude)
    for name in _ARG_OF:
        if _TYPE_OF.get(name) == "columns":
            spoken_for.update(column_list(values.get(name)))
    return [c for c in table.select_dtypes("number").columns
            if c not in spoken_for]


def build(values: dict[str, Any], table, px,
          exclude: Iterable[str] = ()) -> tuple[dict[str, Any], list[str]]:
    """Keyword arguments for `px.<kind>(table, **kwargs)`.

    `px` is `plotly.express`, passed in rather than imported: this module
    is part of the Qt-free, import-light core and a node's `run()` already
    has it to hand. It is used only to resolve a named colour palette,
    which plotly takes as a list of colours rather than by name.
    ...
    accepted = KIND_ARGS[kind] | UNIVERSAL_ARGS
...
def _fill_defaults(kwargs, values, table, kind, accepted,
    ...
    if "dimensions" in accepted and "dimensions" not in kwargs:
        ...
        picked = ([c for c in table.columns if c not in set(exclude)]
                  ...
                  else default_columns(table, values, exclude))
        if picked:
    ...
    if kind in DISTRIBUTION_KINDS:
        ...
        if "y" in kwargs or "y" not in accepted:
            return
        axis = "y"
    picked = default_columns(table, values, exclude)
    if not picked:
...
def _check_columns(kwargs, columns, kind) -> None:
    ...
    for arg, value in kwargs.items():
        if arg not in ROLE_ARGS and arg not in _COLUMN_ARGS:
            continue
...
_ROLE_LABELS: dict[str, str] = {r["arg"]: r["label"] for r in _ROWS
                                if r.get("arg") in ROLE_ARGS}


def layout_updates(values: dict[str, Any], kind: str) -> dict[str, Any]:
    ...
    if barnorm != "none" and "barnorm" not in KIND_ARGS.get(kind, ()):
        out["barnorm"] = barnorm
    return out
