...
one appearing only for the chart kinds that have it, in the same folding
sections, and the styling sections — legend, axis
...
down to `_apply_styling` — the same 28 chart kinds, the same settings, and
the same styling block — by construction, not by sharing code, so anything
added here has to be added there too. That trade was deliberate: importing
this from a shared module used to keep the two in lockstep automatically —
see `tests/test_plotly_kinds.py` and `tests/test_plotly_styling.py`, which
check the two copies agree, and `tests/test_plotly_spec.py`, which checks
each copy's `_KIND_ARGS` table against the plotly actually installed.
...
def _panel_rows(compare, split_by: str, value):
    """The compare table cut to this panel, when it has the split column.

    A compare table without it is used whole — a single historic total is a
    reasonable thing to draw on every panel.
    """
    if compare is None or split_by not in getattr(compare, "columns", ()):
        return compare
    return compare[compare[split_by] == value]
...
def run(ctx, table, compare=None):
    ...
    for index, (value, group) in enumerate(groups):
        ...
        _apply_styling(figure, ctx.params)
        # Each panel's own rows, and the matching slice of `compare`, so a
        # "total from compare" line follows the split the same way.
        _apply_chart_rules(
            ctx, figure, group, group,
            _panel_rows(compare, split_by, value), kwargs, kind)
