...
from flopy.core import NodeRegistry, fuzzy_score
...
class TestBuiltinDiscovery:
    def test_seed_nodes_load(self, registry):
        ...
        assert {"flopy.util.constant", "flopy.util.reroute",
                "flopy.scripting.python_script", "flopy.transform.filter_rows",
                "flopy.io.read_csv"} <= type_ids
    ...
    def test_instantiate(self, registry):
        node = registry.instantiate("flopy.util.constant", pos=(10, 20))
    ...
    def test_unknown_type(self, registry):
        with pytest.raises(KeyError, match="unknown node type"):
            registry.get("flopy.nope.missing")
...
class TestSeedNodeBehaviour:
    def test_constant(self, registry):
        run_source = registry.get("flopy.util.constant").source
    ...
    def test_filter_rows(self, registry):
        from flopy.core import compile_run
        run = compile_run(registry.get("flopy.transform.filter_rows").source, "n2")
    ...
    def test_filter_rows_empty_query_passes_through(self, registry):
        from flopy.core import compile_run
        run = compile_run(registry.get("flopy.transform.filter_rows").source, "n3")
    ...
    def test_read_csv(self, registry, tmp_path):
        ...
        csv.write_text("a,b\n1,2\n3,4\n")
        run = compile_run(registry.get("flopy.io.read_csv").source, "n4")
    ...
    def test_read_csv_requires_path(self, registry):
        from flopy.core import compile_run
        run = compile_run(registry.get("flopy.io.read_csv").source, "n5")
